...
docker run -d --name sibu-admin --restart always \
  -p 8006:8000 \
  --network sibu-net \
...
# Quick local check
sleep 3
curl -sS http://127.0.0.1:8006/health || true
