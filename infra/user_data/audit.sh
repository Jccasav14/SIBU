...
docker pull jccasav/sibu-audit:qa
docker rm -f sibu-audit || true

docker run -d --name sibu-audit --restart always \
  -p 8004:8000 \
  --network sibu-net \
  ...
  jccasav/sibu-audit:qa

# Quick local check
sleep 3
curl -sS http://127.0.0.1:8004/health || true
